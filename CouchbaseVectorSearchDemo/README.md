...
var collection = vectorStore.GetCollection<string, Glossary>(
    ...
    new CouchbaseQueryCollectionOptions 
    {
        ...
        SimilarityMetric = "cosine"               // Used in ANN_DISTANCE() queries
    }
);
```

The `SimilarityMetric` property is crucial as it determines which distance function will be used in the generated SQL++ queries. When you call `SearchAsync()`, the connector automatically uses this metric in the `ANN_DISTANCE()` function.
...
**Index Creation Timing**: The index is created **after** data ingestion because:
1. **Vector indexes require existing data** to analyze vector distributions and optimize storage
2. **Training phase** - Hyperscale indexes need sample vectors to determine optimal centroids and quantization
3. **Performance** - Creating the index on populated data is more efficient than on empty collections
...
The demo performs two types of searches using the connector's `SearchAsync()` method. These searches will work both **with and without** the Hyperscale index:

- **Without Index**: Couchbase performs a sequential scan of all documents (slower, but functional)
- **With Index**: Couchbase uses the optimized vector index for fast approximate nearest neighbor search

The connector automatically detects available indexes and uses them when present.
...
> **Note**: The distance metric (`'cosine'`) in the `ANN_DISTANCE` function is automatically determined by the `SimilarityMetric` configured in the `CouchbaseQueryCollectionOptions` when the collection was created.
...
- 📦 **Connector Library**: [couchbase-semantic-kernel](https://github.com/Couchbase-Ecosystem/couchbase-semantic-kernel)
- 📘 **Couchbase Vector Search**: [Documentation](https://docs.couchbase.com/server/current/vector-search/vector-search.html)
- 🎓 **Semantic Kernel**: [Microsoft Semantic Kernel](https://learn.microsoft.com/en-us/semantic-kernel/)
- 🔧 **NuGet Package**: [CouchbaseConnector.VectorData](https://www.nuget.org/packages/CouchbaseConnector.VectorData)

## Support

This is a community-maintained example. For issues or questions:
- Open an issue on the [connector repository](https://github.com/Couchbase-Ecosystem/couchbase-semantic-kernel)
